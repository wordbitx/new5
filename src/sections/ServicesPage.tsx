...
export function ServicesPage() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-[#6C6C8A] max-w-2xl mx-auto text-lg">
            Professional digital services to help you grow your business. From social media management to web development, we have got you covered.
          </p>
        </div>
        ...
        >
          {services.map((service) => (
            <div 
              key={service.id}
              className="service-card bg-white rounded-[28px] p-6 border border-[#11114D]/10 hover:-translate-y-1.5 transition-all duration-300 hover:shadow-lg"
            >
              {/* Icon */}
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[#7B3FE4]/10 to-[#7B3FE4]/20 flex items-center justify-center mb-4 text-[#7B3FE4]">
                {getIcon(service.icon)}
              </div>

              {/* Content */}
              <h3 className="text-xl font-bold text-[#11114D] mb-2">
                {service.name}
              </h3>
              <p className="text-[#6C6C8A] text-sm mb-4 line-clamp-2">
                {service.description}
              </p>

              {/* Price */}
              <div className="flex items-baseline gap-1 mb-4">
                <span className="text-2xl font-bold text-[#7B3FE4]">
                  ${service.price}
                </span>
              ...
              <ul className="space-y-2 mb-6">
                {service.features.slice(0, 3).map((feature, index) => (
                  <li key={index} className="flex items-center gap-2 text-sm text-[#6C6C8A]">
                    <Check className="w-4 h-4 text-[#7B3FE4] flex-shrink-0" />
                    <span className="truncate">{feature}</span>
                  </li>
                ))}
                {service.features.length > 3 && (
                  <li className="text-sm text-[#7B3FE4]">
  ...
  className="flex-1 border-[#11114D]/20 text-white hover:bg-[#F6F7FB] hover:text-purple-600"
>
  View Details
</Button>
                <Button 
        ...
        <DialogContent className="max-w-lg bg-white max-h-[90vh] overflow-auto">
          {selectedService && (
            <>
              ...
              <div className="bg-[#F6F7FB] rounded-xl p-4 mb-6">
                <div className="flex items-center gap-4 mb-4">
                  <div className="flex items-center gap-2">
                    <DollarSign className="w-5 h-5 text-[#7B3FE4]" />
                    <span className="text-2xl font-bold text-[#7B3FE4]">${selectedService.price}</span>
                  </div>
                  <div className="flex items-center gap-2 text-[#6C6C8A]">
              ...
              <div className="mb-6">
                <h4 className="font-semibold text-[#11114D] mb-3">What's Included:</h4>
              ...
              <div className="flex gap-3">
                <Button 
                  ...
                  className="flex-1 border-[#11114D]/20 text-[#11114D] hover:bg-[#F6F7FB]"
                >
                  Close
                </Button>
                <Button 
